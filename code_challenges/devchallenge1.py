...
# read file from /usr/share/dict/words --> memory
with open('/usr/share/dict/words', 'r') as dictionary_file:
    dictionary = dictionary_file.read()

	# read stdin
	user_word = raw_input('What word would you like spell checked?\n>')
	
	# lowercase word
	word_to_check = user_word.lower()
...
"""
Challenge
Write a program that reads a large list of English words (e.g. from /usr/share/dict/words on a unix system) 
into memory, and then reads words from stdin, and prints either the best spelling suggestion, or 
"NO SUGGESTION" if no suggestion can be found. The program should print ">" as a prompt before reading 
each word, and should loop until killed.
...
starting with correctly spelled English words. Pipe its output into the first program and verify that 
